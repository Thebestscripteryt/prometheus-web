		...
		if(node.kind == AstKind.InterpolatedStringExpression) then
			...
			for _, part in ipairs(node.parts) do
				if(part.kind == "string" and #part.value > 0) then
					data.scope:addReferenceToHigherScope(scope, stringsVar);
					data.scope:addReferenceToHigherScope(scope, decryptVar);
					local encrypted, seed = Encryptor.encrypt(part.value);
					part.kind = "expr";
					part.node = Ast.IndexExpression(Ast.VariableExpression(scope, stringsVar), Ast.FunctionCallExpression(Ast.VariableExpression(scope, decryptVar), {
						Ast.StringExpression(encrypted), Ast.NumberExpression(seed),
					}));
				end
			end
