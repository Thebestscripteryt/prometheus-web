...
-- Parses a LuaU interpolated string, e.g. `Hello {name}, you are {age} years old`
-- Returns a token whose value is a table of parts:
--   { kind = "string", value = "..." } or { kind = "expr", source = "..." }
function Tokenizer:interpolatedString()
	...
	local function flushBuffer()
		parts[#parts + 1] = { kind = "string", value = table.concat(buffer) };
		buffer = {};
	end
	...
	while (not is(self, "`")) do
		...
		if(char == "{") then
			...
			-- Capture the raw source of the expression by tracking brace depth
			...
			while depth > 0 do
				...
				elseif c == "}" then
					depth = depth - 1;
				elseif c == '"' or c == "'" then
					-- Skip over nested string literals so braces inside them don't confuse depth tracking
					local quote = c;
					while not is(self, quote) do
						local sc = get(self);
						if sc == "\\" then
							get(self);
						end
					...
					get(self);
				elseif c == Tokenizer.EOF_CHAR then
					logger:error(generateError(self, "Unterminated interpolated expression"));
				end
			end
			local exprSource = self.source:sub(exprStart + 1, self.index - 1);
			parts[#parts + 1] = { kind = "expr", source = exprSource };
		elseif(char == "\\") then
			char = get(self);
			local escape = self.EscapeSequences[char];
			if(type(escape) == "string") then
	...
	return token(self, startPos, Tokenizer.TokenKind.InterpolatedString, parts);
end
...
function Tokenizer:next()
	...
	-- LuaU Interpolated String Literals (`...{expr}...`)
	if(self.luaVersion == LuaVersion.LuaU and is(self, "`", 0)) then
